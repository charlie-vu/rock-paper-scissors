    ...
    return (
        <>
            <div className="ratio ratio-1x1 mx-auto" style={{ maxWidth: 540 }}>
                ...
                <img src={`/images/bg-${isBonusMode ? 'pentagon' : 'triangle'}.svg`} alt="game-board" className="top-50 start-50 translate-middle z-index-n1" style={{ width: radius * 2, height: 'auto' }} />

                {choiceList.map((type, i) => {
                    const angle = (360 / choiceList.length) * i + (isBonusMode ? -90 : -150);
                    const x = radius * Math.cos((angle * Math.PI) / 180);
                    const y = radius * Math.sin((angle * Math.PI) / 180) + (isBonusMode ? 0 : screen?.gt?.sm ? -60 : -30);
